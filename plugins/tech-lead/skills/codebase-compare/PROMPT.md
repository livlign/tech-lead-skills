...
> Analyze a codebase using two complementary approaches (Claude default tools vs ast-graph + FalkorDB) and produce a shareable summary.html with scan metrics, agreed repo stats, top hotspots, and a tool-per-action cheatsheet. Use when the user wants a codebase overview, benchmark, or ast-graph demo.
